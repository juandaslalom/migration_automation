...
def run_dashboard_cli_commands(site_name: str, base_drive: str, log_folder_path: str, test_mode: bool = False, username: str = None, password: str = None) -> dict:
    ...
    try:
        ...
        if test_mode:
            ...
            # If credentials supplied, run locally to allow interactive login via stdin
            if username and password:
                ...
                try:
                    # Derive remote host from base_drive if it's UNC, else from base_drive root
                    base_path_val = base_drive
                    remote_host = None
                    if base_path_val.startswith('\\'):
                        parts = base_path_val.lstrip('\\').split('\\')
                        if len(parts) >= 2:
                            remote_host = parts[0]
